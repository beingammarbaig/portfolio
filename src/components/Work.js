...
const tagStyle = {
  ...
  Advanced:  "text-[#b936ee] bg-[#b936ee]/10 border-[#b936ee]/30",
  Proficient:"text-[#6366F1] bg-[#6366F1]/10 border-[#6366F1]/30",
};
      ...
      <div className="container mx-auto">
        ...
        <div className="flex flex-col gap-x-12 lg:flex-row mb-10 lg:items-center">
          ...
          >
            ...
            <h3 className="text-[17px] lg:text-[20px] font-primary font-semibold mb-3 text-white">Every Day is a New Challenge</h3>
            <p className="text-justify text-[13px] lg:text-[15px] text-white/80 leading-relaxed mb-5">
              ...
              {radialSkills.map(({ label, pathClass, end }, i) => (
                <motion.div key={i} variants={scaleIn(0.1 * i)} className="flex-1 flex flex-col items-center">
                  <div className="radial-bars">
                    ...
                    <div className="percentage text-white">
                      {inView ? <CountUp start={0} end={end} duration={2} /> : 0}%
                    </div>
                  </div>
                  <h4 className="mt-[-20px] text-center text-[10px] lg:text-[13px] tracking-wide font-primary font-semibold whitespace-pre-line leading-tight text-white/80">
                    {label}
                  </h4>
                </motion.div>
              ))}
              ...
              {skillLevels.map(({ label, level, tag, note }, i) => (
                ...
                >
                  <div className="text-left">
                    <h4 className="text-[9px] sm:text-[10px] lg:text-[13px] tracking-wide font-primary font-semibold leading-snug text-white">{label}</h4>
                    <p className="text-[8px] lg:text-[9px] text-gray-400 mt-0.5 leading-none">{note}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="flex gap-1">
                      {[1,2,3,4,5].map(dot => (
                        <div key={dot} className={`w-3 h-3 lg:w-4 lg:h-4 rounded-sm transition-all duration-300 ${dot <= level ? 'bg-gradient-to-r from-[#ff56f6] to-[#b936ee] shadow-[0_0_6px_rgba(255,86,246,0.35)]' : 'bg-white/10 border border-white/20'}`}></div>
                      ))}
                    </div>
        ...
        >
          <div className="flex items-center gap-3 justify-center mb-6">
            <h3 className="text-[16px] lg:text-[20px] font-primary font-semibold text-white">Courses Taught</h3>
            <span className="text-[10px] font-primary text-white/60 bg-white/10 border border-white/20 rounded-full px-3 py-0.5">8 Courses · 1000+ Students</span>
          </div>
          <motion.div
          ...
          >
            {courseCategories.map((cat, ci) => (
              <motion.div
                ...
                className="bg-black/30 border border-white/10 rounded-xl p-4 hover:border-white/20 hover:shadow-md transition-all duration-300 shadow-sm"
              >
                <div className={`inline-block text-[9px] font-primary font-semibold tracking-widest uppercase mb-3 px-2 py-0.5 rounded bg-gradient-to-r ${cat.color} bg-clip-text text-transparent border border-white/10`}>
                  {cat.category}
                </div>
                <ul className="space-y-2">
                  {cat.courses.map((course, i) => (
                    <li key={i} className="flex items-start gap-2 text-[11px] lg:text-[12px] font-secondary text-white/80">
